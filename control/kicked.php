...
<script data-cfasync="false">
    var waliTime = <?php echo time(); ?>;
    waliCountDown = function() {
        var kickEnd = <?php echo $data['user_kick']; ?>;
        if (waliTime >= kickEnd) {
            location.reload();
        } else {
            var remain = kickEnd - waliTime;
    ...
    $(document).ready(function() {
        waliKickTimer = setInterval(waliCountDown, 1000);
        waliCheckKick = setInterval(checkKick, 30000);
        waliCountDown();
        checkKick();
    });
</script>