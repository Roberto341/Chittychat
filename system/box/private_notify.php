...
return '<div class="ulist_item" >
			<div class="ulist_avatar">
				<img src="' . myAvatar($p['user_avatar']) . '"/>
			</div>
			<div class="ulist_name gprivate" data="' . $p['user_id'] . '" value="' . $p['user_name'] . '" data-av="' . myAvatar($p['user_avatar']) . '">
...
$private = $mysqli->query("
	SELECT count(*) as private_count,  MAX(a.time) AS time, a.hunter, b.user_id, b.user_name, b.user_color, b.user_avatar from wali_private a inner join wali_users b on a.hunter = b.user_id 
	WHERE status = 0 AND target = {$data['user_id']} 
	GROUP BY hunter,user_id,user_name,user_color,user_avatar
	ORDER BY time ASC
");
...
if($new_count < $notify_limit){
	...
	$get_other = $mysqli->query("
		SELECT MAX(a.time) AS time, a.hunter, b.user_id, b.user_name, b.user_color, b.user_avatar from wali_private a inner join wali_users b on a.hunter = b.user_id 
