...
(SELECT count(id) FROM wali_news) as news_count,
...
WHERE wali_news.news_poster = wali_users.user_id 
...
if ($get_news->num_rows > 0) {
    ...
    $mysqli->query("UPDATE wali_users SET user_news = '" . time() . "' WHERE user_id = '{$data['user_id']}'");
} else {
