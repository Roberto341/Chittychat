...
$wall_content = '';
$post_count = 0;
$find_friend = $mysqli->query("SELECT target FROM wali_friends WHERE hunter = '{$data['user_id']}' AND fstatus = '3'");
...
$wall_post = $mysqli->query("SELECT wali_post.*, wali_users.*,
(SELECT count( parent_id ) FROM wali_post_reply WHERE parent_id = wali_post.post_id ) as reply_count,
(SELECT like_type FROM wali_post_like WHERE uid = '{$data['user_id']}' AND like_post = wali_post.post_id) as liked,
(SELECT count( post_id ) FROM wali_post WHERE post_user IN ($newarray)) as post_count
FROM  wali_post, wali_users 
WHERE wali_post.post_user = wali_users.user_id AND wali_post.post_user IN ($newarray)
ORDER BY wali_post.post_actual DESC LIMIT 10");

if ($wall_post->num_rows > 0) {
    while ($wall = $wall_post->fetch_assoc()) {
        $post_count = $wall['post_count'];
        $wall_content .= waliTemplate('element/wall_post', $wall);
    }
} else {
